...
1. `npm run build`, then start `ADMIN_CODE=<test code> PORT=8787 node dist-server/index.js` in the
   ...
   `npm run upload -- --key <POOL_KEY> --url http://localhost:8787 photos/<file>`.
2. Load the `claude-in-chrome` skill, open http://localhost:8787 and open the pool with its key (Pool home, then Play).
